const addMessage = function (msg, senderId) {
    // const messageClass = senderId;
    // const messageClass = c;
    // const messageClass = senderId == userId ? 'message-out' : 'message-in';
    const messageClass = msg.user_id == userId ? 'message-out' : 'message-in';
...
const getConverations = function () {
                    ...
                    <div class="col">
                        <div class="d-flex align-items-center mb-3">
                            ...
                            <span class="text-muted extra-small ms-2">
                                ${moment(chat.last_message.created_at).fromNow()}</span>
                        </div>
...
$('#chat-list').on('click', '[data-messages]', function (e) {
    ...
    $.get(`/api/conversation/${id}/messages`, function (response) {
        ...
        for (i in response.messages.data) {
            ...
            // addMessage(response.data[i], response.data[i].user_id);
            addMessage(msg, c);
        }
    })
})
